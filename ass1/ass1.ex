...
defmodule A1 do
  ...
  defp base256_to_base10(n, currDigitPlace, acc) do
    case n do
      [] ->  trunc(acc)
      [h | t] -> 
  ...
  def binary_to_integer(bin), do: base256_to_base10(:binary.bin_to_list(bin), String.length(bin) - 1, 0)
  ...
  def crypt(text, {k, n}) do
    p = 12945845173329038168894514532852761087184861453819103626524746458007544079436193033296354255392833659165781825320450882527971501424543610656635242495207243
    q = 11651088227042719432197879712448441990940913399774434709130882531920046670720814447752206963062868458438310623799329703497031760471331490496164292198579167
    e = 65537
    ...
    if k == e do
      val = binary_to_integer(text)
      powm(val, k, n) |> integer_to_binary() 
    end
    if k == d do 
      val = binary_to_integer(text)
      powm(val, d, n) |> integer_to_binary()
    end
  end
end
